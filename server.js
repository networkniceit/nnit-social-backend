...
app.post('/api/ai/video-script', async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({ success: true, script: { raw: text } });
    }
  } catch (err) {
    console.error('video-script error:', err.response?.data || err.message);
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/ai/photo-prompts', async (req, res) => {
  try {
    ...
    console.error('photo-prompts error:', err.response?.data || err.message);
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/analytics/:clientId', async (req, res) => {
  try {
    const { clientId } = req.params;
    const postsResult = await pool.query('SELECT * FROM posts WHERE client_id = $1 ORDER BY created_at DESC LIMIT 10', [clientId]).catch(() => ({ rows: [] }));
    const totalPosts = postsResult.rows.length;
    res.json({
      success: true,
      analytics: {
