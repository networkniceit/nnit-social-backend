...
app.get('/api/auth/twitter', (req, res) => {
  ...
  app.locals.twitterCodeVerifier = codeVerifier;
  app.locals.twitterState = state;
  const authUrl = `https://twitter.com/i/oauth2/authorize?response_type=code&client_id=${TWITTER_CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&scope=tweet.read%20tweet.write%20users.read%20offline.access&state=${state}&code_challenge=${codeVerifier}&code_challenge_method=plain`;
  res.redirect(authUrl);
});

app.get('/api/auth/twitter/callback', async (req, res) => {
  const { code, error } = req.query;
  if (error || !code) return res.redirect(`${process.env.FRONTEND_URL}/settings?twitter_error=true&reason=${encodeURIComponent(error || 'no_code')}`);
  try {
    const TWITTER_CLIENT_ID = process.env.TWITTER_CLIENT_ID;
    const TWITTER_CLIENT_SECRET = process.env.TWITTER_CLIENT_SECRET;
    const REDIRECT_URI = `${process.env.BACKEND_URL}/api/auth/twitter/callback`;
    ...
    res.redirect(`${process.env.FRONTEND_URL}/settings?twitter_connected=true&twitter_username=${encodeURIComponent(user.username)}&twitter_name=${encodeURIComponent(user.name)}&twitter_id=${user.id}`);
  } catch (error) {
    console.error('Twitter OAuth error:', error.response?.data || error.message);
    res.redirect(`${process.env.FRONTEND_URL}/settings?twitter_error=true&reason=${encodeURIComponent(JSON.stringify(error.response?.data) || error.message)}`);
  }
...
app.get('/api/auth/twitter/load', async (req, res) => {
  try {
    const result = await pool.query(`SELECT * FROM social_accounts WHERE user_id = $1 AND platform = 'twitter'`, [req.query.userId || 1]);
    if (result.rows.length > 0) res.json({ success: true, account: result.rows[0] });
    else res.json({ success: false, account: null });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
...
app.delete('/api/auth/twitter/disconnect', async (req, res) => {
  try {
    await pool.query(`DELETE FROM social_accounts WHERE user_id = $1 AND platform = 'twitter'`, [req.query.userId || 1]);
    res.json({ success: true, message: 'Twitter disconnected' });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
app.post('/api/twitter/post', async (req, res) => {
  try {
    const { text, userId } = req.body;
    const dbResult = await pool.query(`SELECT access_token FROM social_accounts WHERE user_id = $1 AND platform = 'twitter'`, [userId || 1]);
    if (dbResult.rows.length === 0) return res.status(400).json({ success: false, error: 'Twitter not connected' });
    const { access_token } = dbResult.rows[0];
    const response = await axios.post('https://api.twitter.com/2/tweets', { text }, { headers: { 'Authorization': `Bearer ${access_token}`, 'Content-Type': 'application/json' } });
    res.json({ success: true, tweetId: response.data.data.id });
  } catch (error) {
    res.status(500).json({ success: false, error: error.response?.data?.detail || error.message });
  }
...
app.get('/api/twitter/analytics', async (req, res) => {
  try {
    const dbResult = await pool.query(`SELECT access_token, instagram_account_name, page_id FROM social_accounts WHERE user_id = $1 AND platform = 'twitter'`, [req.query.userId || 1]);
    if (dbResult.rows.length === 0) return res.status(400).json({ success: false, error: 'Twitter not connected' });
    const { access_token, instagram_account_name: displayName, page_id: username } = dbResult.rows[0];
    const userResponse = await axios.get('https://api.twitter.com/2/users/me', {
      headers: { 'Authorization': `Bearer ${access_token}` },
    ...
    const user = userResponse.data.data;
    const metrics = user.public_metrics || {};
    res.json({ success: true, username: username || displayName, displayName, followerCount: metrics.followers_count || 0, followingCount: metrics.following_count || 0, tweetCount: metrics.tweet_count || 0, profileImageUrl: user.profile_image_url || '' });
  } catch (error) {
    res.status(500).json({ success: false, error: error.response?.data?.detail || error.message });
  }
});
