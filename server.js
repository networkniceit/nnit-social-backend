...
app.get('/api/auth/instagram/callback', async (req, res) => {
  ...
  try {
    ...
    // Step 3: Get Instagram Business Account linked to page
    const igAccountUrl =
    ...
    try {
      await pool.query(`
        INSERT INTO social_accounts (
          ...
          instagram_account_id, instagram_account_name,
    ...
    // Step 7: Redirect to frontend with credentials
    res.redirect(
      `${process.env.FRONTEND_URL}/settings?` +
      `instagram_connected=true` +
      `&access_token=${pageAccessToken}` +
    ...
    console.error('Instagram OAuth error:', error.response?.data || error.message);
    res.redirect(
      `${process.env.FRONTEND_URL}/settings?instagram_error=true` +
      `&reason=${encodeURIComponent(error.response?.data?.error?.message || error.message)}`
    );
  }
});
...
app.get('/api/auth/instagram/load', async (req, res) => {
  try {
    ...
    console.error('Instagram load error:', error.message);
    res.status(500).json({ success: false, error: error.message });
  }
});

// ================================================================
// CLIENT PLATFORM CONNECTION ROUTES
// ================================================================
...
Return as JSON object: {"variations": ["variation1", "variation2", "variation3"]}`;
...
Return as JSON: {"ideas": ["idea1", "idea2", ...]}`;
    ...
    console.error('Content ideas error:', error.message);
    res.status(500).json({ error: error.message });
  }
});
...
app.get('/api/auth/facebook/callback', async (req, res) => {
  ...
  try {
    const { FACEBOOK_APP_ID, FACEBOOK_APP_SECRET, BACKEND_URL, FRONTEND_URL } = process.env;
    ...
    if (!pagesResponse.data.data || pagesResponse.data.data.length === 0) {
      return res.redirect(`${FRONTEND_URL}/settings?facebook_error=true&reason=no_pages`);
    }

    const { access_token: pageAccessToken, id: pageId, name: pageName } = pagesResponse.data.data[0];
    ...
    await pool.query(`
      ...
      ON CONFLICT (user_id, platform) DO UPDATE SET
        access_token = $2, instagram_account_name = $3,
        page_id = $4, page_access_token = $5,
        updated_at = CURRENT_TIMESTAMP
    `, [1, userAccessToken, pageName, pageId, pageAccessToken]);
    ...
    console.error('Facebook OAuth error:', error.response?.data || error.message);
    const reason = encodeURIComponent(error.response?.data?.error?.message || error.message);
    res.redirect(`${process.env.FRONTEND_URL}/settings?facebook_error=true&reason=${reason}`);
  }
});
...
app.get('/api/auth/facebook/load', async (req, res) => {
  try {
    ...
    res.json(result.rows.length > 0
      ? { success: true, account: result.rows[0] }
      : { success: false, account: null }
    );
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
  ...
  try {
    ...
    const result = await pool.query(`
      ...
      ON CONFLICT (user_id, platform) DO UPDATE SET
        ...
        updated_at = CURRENT_TIMESTAMP
      RETURNING *
    `, [resolvedUserId, accessToken, pageName, pageId, pageAccessToken]);
    res.json({ success: true, account: result.rows[0] });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
app.post('/api/facebook/post/photo', async (req, res) => {
  try {
    ...
    const { page_id, page_access_token } = dbResult.rows[0];
    const response = await axios.post(`https://graph.facebook.com/v18.0/${page_id}/photos`, {
      caption,
      url: imageUrl,
      access_token: page_access_token
    });
...
app.get('/api/facebook/analytics', async (req, res) => {
  try {
    ...
    const { page_id, page_access_token, instagram_account_name: storedPageName } = dbResult.rows[0];

    const [insightsResponse, pageInfoResponse] = await Promise.all([
      axios.get(`https://graph.facebook.com/v18.0/${page_id}/insights?metric=${metric}&period=${period}&access_token=${page_access_token}`),
      axios.get(`https://graph.facebook.com/v18.0/${page_id}?fields=fan_count,followers_count,name&access_token=${page_access_token}`)
    ]);
    ...
    res.json({
      success: true,
      pageName: pageInfoResponse.data.name || storedPageName,
      fanCount: pageInfoResponse.data.fan_count,
      followersCount: pageInfoResponse.data.followers_count,
      insights
    });
...
app.get('/api/facebook/analytics/post/:postId', async (req, res) => {
  try {
    ...
    const { page_access_token } = dbResult.rows[0];
    const response = await axios.get(
      `https://graph.facebook.com/v18.0/${req.params.postId}/insights` +
...
app.get('/api/auth/tiktok/callback', async (req, res) => {
  ...
  if (error || !code) {
    return res.redirect(
      `${process.env.FRONTEND_URL}/settings?tiktok_error=true&reason=${encodeURIComponent(error || 'no_code')}`
    );
  }

  try {
    const { TIKTOK_CLIENT_KEY, TIKTOK_CLIENT_SECRET, BACKEND_URL, FRONTEND_URL } = process.env;
    const REDIRECT_URI = `${BACKEND_URL}/api/auth/tiktok/callback`;
    ...
    await pool.query(`
      ...
      ON CONFLICT (user_id, platform) DO UPDATE SET
        access_token = $2, instagram_account_id = $3, instagram_account_name = $4,
        page_id = $5, page_access_token = $6,
        updated_at = CURRENT_TIMESTAMP
    `, [1, access_token, open_id, displayName, open_id, refresh_token]);
    ...
    console.error('TikTok OAuth error:', error.response?.data || error.message);
    const reason = encodeURIComponent(error.response?.data?.message || error.message);
    res.redirect(`${process.env.FRONTEND_URL}/settings?tiktok_error=true&reason=${reason}`);
  }
});
...
app.get('/api/tiktok/analytics', async (req, res) => {
  try {
    ...
    const { access_token, instagram_account_name: storedDisplayName } = dbResult.rows[0];
    const userResponse = await axios.get('https://open.tiktokapis.com/v2/user/info/', {
      headers: { 'Authorization': `Bearer ${access_token}` },
      params: { fields: 'open_id,display_name,follower_count,following_count,video_count,profile_deep_link' }
    });
    ...
    res.json({
      success: true,
      username: userInfo.display_name || storedDisplayName,
...
app.get('/api/auth/twitter/callback', async (req, res) => {
  const { code, error } = req.query;
  const { TWITTER_CLIENT_ID, TWITTER_CLIENT_SECRET, BACKEND_URL, FRONTEND_URL } = process.env;
  if (error || !code) {
    return res.redirect(`${FRONTEND_URL}/settings?twitter_error=true&reason=${encodeURIComponent(error || 'no_code')}`);
  }
  try {
    const REDIRECT_URI = `${BACKEND_URL}/api/auth/twitter/callback`;
    ...
    await pool.query(`
      ...
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      ON CONFLICT (user_id, platform) DO UPDATE SET
        access_token = $3, instagram_account_id = $4, instagram_account_name = $5,
        page_id = $6, page_access_token = $7, updated_at = CURRENT_TIMESTAMP
    `, [1, 'twitter', access_token, user.id, user.name, user.username, refresh_token || '']);
    res.redirect(
      `${FRONTEND_URL}/settings?twitter_connected=true` +
      `&twitter_username=${encodeURIComponent(user.username)}` +
      `&twitter_name=${encodeURIComponent(user.name)}` +
      `&twitter_id=${user.id}`
    );
  } catch (error) {
    console.error('Twitter OAuth error:', error.response?.data || error.message);
    res.redirect(`${FRONTEND_URL}/settings?twitter_error=true&reason=${encodeURIComponent(JSON.stringify(error.response?.data) || error.message)}`);
  }
});
...
app.get('/api/auth/youtube', (req, res) => {
  const { YOUTUBE_CLIENT_ID, BACKEND_URL } = process.env;
...
app.get('/api/auth/youtube/callback', async (req, res) => {
  const { code, error } = req.query;
  const { YOUTUBE_CLIENT_ID, YOUTUBE_CLIENT_SECRET, BACKEND_URL, FRONTEND_URL } = process.env;
  if (error || !code) {
    return res.redirect(`${FRONTEND_URL}/settings?youtube_error=true&reason=${encodeURIComponent(error || 'no_code')}`);
  }
  try {
    const REDIRECT_URI = `${BACKEND_URL}/api/auth/youtube/callback`;
    const tokenResponse = await axios.post(
    ...
    res.redirect(
      `${FRONTEND_URL}/settings?youtube_connected=true` +
      `&youtube_channel=${encodeURIComponent(channelTitle)}` +
      `&youtube_id=${channelId}`
    );
  } catch (error) {
    console.error('YouTube OAuth error:', error.response?.data || error.message);
    res.redirect(`${FRONTEND_URL}/settings?youtube_error=true&reason=${encodeURIComponent(JSON.stringify(error.response?.data) || error.message)}`);
  }
});
